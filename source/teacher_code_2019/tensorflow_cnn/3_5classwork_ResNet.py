...
class ResNet50(object):
    def __init__(self, inputs, num_classes=***, is_training=True,
                 scope="resnet50"):
        self.inputs =inputs
        ...
        with tf.variable_scope(scope):
            # construct the model
            net = conv2d(inputs, ***, ***, ***, scope="conv1") # -> [batch, 112, 112, 64]
            net = tf.nn.relu(batch_norm(net, is_training=self.is_training, scope="bn1"))
            net = max_pool(net, ***, ***, scope="maxpool1")  # -> [batch, 56, 56, 64]
            net = self._block(net, ***, ***, init_stride=1, is_training=self.is_training,
                              scope="block2")           # -> [batch, 56, 56, 256]
            net = self._block(net, ***, ***, is_training=self.is_training, scope="block3")
                                                        # -> [batch, 28, 28, 512]
            net = self._block(net, ***, ***, is_training=self.is_training, scope="block4")
                                                        # -> [batch, 14, 14, 1024]
            net = self._block(net, ***, ***, is_training=self.is_training, scope="block5")
                                                        # -> [batch, 7, 7, 2048]
